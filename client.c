...
void close_server_connection() {
    ...
    int resultFromClose = close(serverSocket);
    if(resultFromClose == -1) {
        perror("close");
        exit(1);
    }
    // printf(">>> Done\n");
...
int connect_to_server(const char *host, const char *port) {
    ...
    /* Remember to clear addrinfo struct! */
    ...
    int resultFromGAI = getaddrinfo(host, port, &hints, &result);
    if(resultFromGAI != 0) {
        // Something went wrong! Notice gai_strerror()
    ...
    freeaddrinfo(result);  /* Don't need it */
    result = NULL;
